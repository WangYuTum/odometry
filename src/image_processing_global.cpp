...
GlobalStatus GaussianImagePyramidNaive(int num_levels, const cv::Mat& in_img, std::vector<cv::Mat>& out_pyramids, bool smooth){
  ...
  // necessary checks
  if (rows % 2 != 0 || cols % 2 !=0 || channels != 2 || in_img.type() != PixelType){
  ...
  out_pyramids[0].create(rows, cols, PixelType);
  if (smooth == true){
  ...
  rows = rows / 2;
  cols = cols / 2;
  out_pyramids[1].create(rows, cols, PixelType);
  ...
  for (int l = 2; l < num_levels; l++){
    out_pyramids[l].create(rows, cols, PixelType);
