...
LevenbergMarquardtOptimizer::LevenbergMarquardtOptimizer(float lambda,
                                                         ...
                                                         const int kRobustEst,
                                                         const float kHuberDelta){
  lambda_ = lambda;
  ...
  robust_est_ = kRobustEst;
  huber_delta_ = kHuberDelta;
}
...
OptimizerStatus LevenbergMarquardtOptimizer::ComputeResidualJacobianNaive(const cv::Mat& kImg1,
  ...
  for (int y = 4; y < kRows - 4; y++){ // ignore boundary by 4 pixels
    for (int x = 4; x < kCols - 4; x++){ // ignore boundary by 4 pixels
      // skip invalid depth, <= 10 cm to the camera
      if (kDep1.at<float>(y, x) - 0.0f < 0.10f) {
        num_invalid_dep++;
        continue;
      } else{
        left_coord << x, y, kDep1.at<float>(y, x), 1.0f;
        ...
        if (warp_flag == -1) { // out of image boundary or too close to the camera after warping
          num_out_bound++;
          continue;
        }
...
OptimizerStatus LevenbergMarquardtOptimizer::Reset(const Affine4f& kRelativeInit, const float kLambda){
  OptimizerStatus status_set_init = SetInitialAffine(kRelativeInit);
  OptimizerStatus status_set_lambda = SetLambda(kLambda);
...
OptimizerStatus LevenbergMarquardtOptimizer::SetLambda(const float kLambda = 0.001){
  lambda_ = kLambda;
  // return -1;
  return 0;
}
